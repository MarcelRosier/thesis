...
def get_basic_net_16_16_16(num_input_channels=1, c_hid=16, act_fn=nn.GELU, latent_dim=4096):
    ...
    linear_basic = nn.Sequential(
        nn.Linear(latent_dim, linear_layer_size),
        act_fn()
    )
...
def legacy_basic_16(num_input_channels=1, c_hid=16, act_fn=nn.GELU, latent_dim=4096):
    """Best appraoch (so far)"""
    linear_layer_size = 3 * 16*16*16 * c_hid
    encoder_basic = nn.Sequential(
        nn.Conv3d(num_input_channels, c_hid, kernel_size=3,
...
def legacy_basic_16_wo_last_linear(num_input_channels=1, c_hid=16, act_fn=nn.GELU, latent_dim=4096):
    """Best appraoch (so far)"""
    linear_layer_size = 3 * 16*16*16 * c_hid
    encoder_basic = nn.Sequential(
        nn.Conv3d(num_input_channels, c_hid, kernel_size=3,
...
def get_basic_net_16_16_16_without_last_linear(num_input_channels=1, c_hid=16, act_fn=nn.GELU, latent_dim=4096):
    """
    same as get_basic_net_16_16_16 only without the linear layer
    from the flattened layer to the latent dim,
    needed for the monai varautoencoder structure 
    """
    linear_layer_size = 3 * 16*16*16 * c_hid
    encoder_basic = nn.Sequential(
        nn.Conv3d(num_input_channels, c_hid, kernel_size=3,
