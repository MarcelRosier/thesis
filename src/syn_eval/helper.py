...
def run_1k():
    ...
    for tid in ids:
        ...
        # vae[tid] = utils.find_n_best_score_ids(
        #     path=f"{base_path}/vae1024/{tid}.json", value_type=utils.DSValueType.COMBINED, order_func=min, n_best=15)
        hash_ae[tid] = utils.find_n_best_score_ids(
            path=f"{base_path}/hash_ae_1024/{tid}.json", value_type=utils.DSValueType.COMBINED, order_func=min, n_best=15)
    ...
    # with open(f"{sub_path}/vae1024.json", 'w') as file:
    #     json.dump(vae, file)
    with open(f"{sub_path}/hash_ae_1024.json", 'w') as file:
        json.dump(hash_ae, file)
...
def run_real_lists():
    ...
    for tid in tumor_ids:
        baseline[tid] = utils.find_n_best_score_ids(
            path=f"/home/ivan_marcel/thesis/src/baseline/data/custom_dice/testset_size_50000/dim_128/dice/{tid}.json", value_type=utils.DSValueType.COMBINED, order_func=max, n_best=15)
        # down64[tid] = utils.find_n_best_score_ids(
        #     path=f"{base_path}/down64/{tid}.json", value_type=utils.DSValueType.COMBINED, order_func=max, n_best=15)
        # down32[tid] = utils.find_n_best_score_ids(
        #     path=f"{base_path}/down32/{tid}.json", value_type=utils.DSValueType.COMBINED, order_func=max, n_best=15)
        ...
        #     path=f"{base_path}/vae1024/{tid}.json", value_type=utils.DSValueType.COMBINED, order_func=min, n_best=15)
        hash_ae[tid] = utils.find_n_best_score_ids(
            path=f"/home/ivan_marcel/thesis/src/autoencoder/data/hash_enc_sim/{tid}.json", value_type=utils.DSValueType.COMBINED, order_func=min, n_best=15)
    ...
    #     json.dump(vae, file)
    with open(f"{base_path}/hash_ae_1024.json", 'w') as file:
        json.dump(hash_ae, file)
