...
import numpy as np
import utils
from constants import SYN_TUMOR_PATH_TEMPLATE, ENV
import torch
...
def run_query_for_encoded_data(syn_eval_tumor_id, use_stored_real_data=True, is_ae=True):
    ...
    if use_stored_real_data:
        syn_eval_tumor_t1c = utils.normalize(
            np.load(f"{base_path_t1c}/syn_eval/{syn_eval_tumor_id}.npy"))
        syn_eval_tumor_flair = utils.normalize(
            np.load(f"{base_path_flair}/syn_eval/{syn_eval_tumor_id}.npy"))
    else:
        if not is_ae:
